...
function initSliders() {
  if (document.querySelector(".swiper")) {
    let updateCenter2 = function(sw) {
      sw.slides.forEach((slide2) => {
        const inner = slide2.querySelector(".dishes__inner");
        const body = slide2.querySelector(".dishes__body");
        if (inner) {
          inner.classList.remove("is-center");
          inner.setAttribute("data-dishes-value", DEFAULT_VALUE);
        }
        if (body) {
          body.classList.remove("is-center");
        }
      });
      const visible = Array.from(
        sw.el.querySelectorAll(".swiper-slide-visible")
      );
      const mid = Math.floor(visible.length / 2);
      const centerSlide = visible[mid];
      if (!centerSlide) return;
      const centerInner = centerSlide.querySelector(".dishes__inner");
      const centerBody = centerSlide.querySelector(".dishes__body");
      if (centerInner) {
        centerInner.classList.add("is-center");
        centerInner.setAttribute("data-dishes-value", CENTER_VALUE);
      }
      if (centerBody) {
        centerBody.classList.add("is-center");
      }
    };
    var updateCenter = updateCenter2;
    ...
    new Swiper(".swiper", {
      ...
      */
      ...
      on: {
        ...
        // если хочешь менять строго после анимации:
        // slideChangeTransitionEnd() { updateCenter(this); }
      }
    });
  }
}
...
function masonryRun() {
  const grids = document.querySelectorAll("[data-fls-masonry]");
  if (!grids.length) return;
  grids.forEach((grid) => {
    const root = grid.closest(".masonry") || document;
    const filterContainer = root.querySelector(".filter-masonry");
    const DEFAULT_FILTER = "a";
    const iso = new Isotope(grid, {
      ...
      filter: `[data-fls-masonry-filter="${DEFAULT_FILTER}"]`
    });
    const titlehead = root.querySelector("[data-fls-titlehead]");
    const titleLabel = titlehead?.querySelector(".titlehead__lable");
    const titleTitle = titlehead?.querySelector(".titlehead__title");
    const titleText = titlehead?.querySelector(".titlehead__text p");
    function updateTitleFromButton(btn) {
      if (!titlehead) return;
      const { titleheadLabel, titleheadTitle, titleheadText } = btn.dataset;
      if (titleheadLabel !== void 0 && titleLabel)
        titleLabel.textContent = titleheadLabel;
      if (titleheadTitle !== void 0 && titleTitle)
        titleTitle.textContent = titleheadTitle;
      if (titleheadText !== void 0 && titleText)
        titleText.textContent = titleheadText;
    }
    const buttons = [
      ...root.querySelectorAll("[data-fls-masonry-filter-link]")
    ];
    let activeBtn = buttons.find((b) => b.dataset.flsMasonryFilterLink === DEFAULT_FILTER) || buttons[0];
    if (activeBtn) {
      activeBtn.classList.add("filter-masonry__link--active");
      updateTitleFromButton(activeBtn);
    }
    if (filterContainer) {
      filterContainer.addEventListener("click", (e) => {
        const btn = e.target.closest("[data-fls-masonry-filter-link]");
        if (!btn) return;
        const value = btn.dataset.flsMasonryFilterLink;
        iso.arrange({
          filter: value === "*" ? "*" : `[data-fls-masonry-filter="${value}"]`
        });
        activeBtn?.classList.remove("filter-masonry__link--active");
        btn.classList.add("filter-masonry__link--active");
        activeBtn = btn;
        updateTitleFromButton(btn);
        e.preventDefault();
      });
    }
  });
